...
export class RunKRequirementsRegressionUseCase {
  constructor(
  ...
  async execute(input: RunRegressionSuiteInput = {}): Promise<RunRegressionSuiteOutput> {
    ...
    // Filter by kNumberFilter if provided
    let testCases = definition.testCases as KRequirementTest[];
    if (input.kNumberFilter && input.kNumberFilter.length > 0) {
      testCases = testCases.filter((tc) => input.kNumberFilter!.includes(tc.kNumber));
    }

    let runResult;
    try {
      runResult = await this.testRunnerPort.runSuite(testCases);
    } catch (err) {
