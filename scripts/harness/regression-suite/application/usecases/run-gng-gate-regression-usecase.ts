...
export class RunGngGateRegressionUseCase {
  constructor(
  ...
  async execute(input: RunRegressionSuiteInput = {}): Promise<RunRegressionSuiteOutput> {
    const suiteId = SuiteId.create(input.suiteId ?? 'gng-gate');
    const definition = await this.suiteRegistryPort.getDefinition(suiteId);

    const runResult = await this.testRunnerPort.runSuite(definition.testCases);
