...
target("PresetDefinitionStore", () => {
  describe("load", () => {
    ...
    context("World rollout defaultsを確認する場合", () => {
      it("全Presetでautomatic integrationが無効かつcanonical pathであること", () => {
        // Arrange
        const store = new PresetDefinitionStore();
        ...
        // Assert
        expect([actual.minimal.world, actual.standard.world, actual.strict.world]).toEqual([
          expect.objectContaining({ enabled: false }),
          expect.objectContaining({ enabled: false }),
          expect.objectContaining({ enabled: false }),
        ]);
        expect(actual.strict.world?.declarations.constraintsPath).toBe("phasegate.world-constraints.json");
      });
    });
  });
});
