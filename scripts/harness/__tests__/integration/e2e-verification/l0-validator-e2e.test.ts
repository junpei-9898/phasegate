  ...
  context('5層防御モデル', () => {
    it('T-042-01 レジストリにL2-L4のバリデータのみが登録されていること', () => {
      ...
      expect(actual.map((d) => d.validatorId.value)).toEqual([
        ...
        'L3-001', 'L3-002', 'L3-003', 'L3-004', 'L3-005',
        'L4-001', 'L4-002', 'L4-003', 'L4-004', 'L4-005', 'L4-006', 'L4-007',
      ]);
    ...
    it('T-042-02 L2-L4バリデータが引き続き登録されていること', () => {
      ...
      expect(actual.l3Defs.map((d) => d.validatorId.value)).toEqual(['L3-001', 'L3-002', 'L3-003', 'L3-004', 'L3-005']);
      // WI-222 (HF2-05): L4-007 (default-OFF advisory) が registry に登録されている。
      expect(actual.l4Defs.map((d) => d.validatorId.value)).toEqual(['L4-001', 'L4-002', 'L4-003', 'L4-004', 'L4-005', 'L4-006', 'L4-007']);
    });
