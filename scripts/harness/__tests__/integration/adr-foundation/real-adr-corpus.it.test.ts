...
// コーパス正規化（adr-gate-normalization-followup.md）により、旧 `ADR-NNN-*.md` 形式
// （非 YAML）の legacy ADR 18 件は canonical `NNN-slug.md` + YAML frontmatter 形式へ
// 移行済みで、discovery 正規表現 `^[0-9]{3}-[a-z0-9-]+\.md$` に一致し gate から可視になった。
// したがってこのスライスでは:
//   - discoverable な canonical コーパス（001..021）の fail-closed conformance と
//   - legacy ADR-接頭辞ファイルが disk 上にもはや残存しない（正規化完了）invariant
// を SCOPED property として正直に検証する。
// per-AC binding（`@ac`）への昇格（H05-02 AC-1/2/3 = §12 Key Decisions 全件の網羅検証）は
// このスライスの範囲外であり、別タスクとして deferred する。file-level `@story H05-02` のみを
// 担持し、H05-02 AC-1/2/3 は fileFallbackOnly のまま維持する（over-claim を回避）。
