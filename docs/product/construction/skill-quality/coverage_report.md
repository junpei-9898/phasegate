...
|------|------------|--------------|------------|
| 受け入れ基準（AC） | 23 | 3（未0 / 部分2 / 対象外1） | 88.5% |
...
>
> **2026-07-08 反転（WI-241）**: skill-kind taxonomy（lifecycle/advisory + 固定 allowlist pin）により実コーパス適合が 30/30 になり、H12-06-AC-2/AC-3 を カバー に反転。総合 45/62 = 72.6% → **47/62 = 75.8%**（§2 H12-06 参照）。<!-- @work-item-id WI-241 -->
...
| H12-06-AC-2 | v0既存スキルのSKILL.mdが必須構造を満たすことを検証するテストが存在する | `skill-corpus-conformance.test.ts`（@ac H12-06-AC-2: 実 skills/* 30件を本番コードパスでディスクから検証, 30/30 適合） | ✅ カバー済み |
| H12-06-AC-3 | v1新規スキルのSKILL.mdが必須構造を満たすことを検証するテストが存在する | `skill-corpus-conformance.test.ts`（@ac H12-06-AC-3: 同上。advisory 7件は固定 allowlist pin で宣言を制限） | ✅ カバー済み |
...
>
> **AC-2/AC-3 反転（2026-07-08, WI-241 skill-kind taxonomy）**: `kind:` frontmatter による構造要件の kind 条件化（lifecycle=7 セクション fail-closed / advisory=frontmatter+languageMetadata+purpose）を実装し、上記 7 スキルに `kind: advisory` を宣言（セクション捏造なし）。実コーパス適合テスト `skill-corpus-conformance.test.ts` が **30/30 green** になったことを根拠に AC-2/AC-3 を カバー に反転した。advisory を宣言できるスキルは同テストの**固定 allowlist（7 件）**に pin されており、8 件目の自己宣言はテスト fail する（fail 動作は仮宣言で実証済み）。lifecycle 23 件が 7 セクションを全保有することも同テストが恒常検証する（anti-gutting）。<!-- @work-item-id WI-241 -->

H12-06 カバレッジ: **4/4 (実カバー4)**
...
>
> **反転（2026-07-08, WI-241）**: skill-kind taxonomy により実コーパス 30/30 適合を達成し、H12-06-AC-2/3 を カバー に反転。実カバー 21/26 = 80.8% → **23/26 = 88.5%**（§2 H12-06 の反転根拠を参照）。<!-- @work-item-id WI-241 -->
