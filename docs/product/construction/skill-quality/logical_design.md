...
<!-- @work-item-id WI-223 -->
release-publisherとskill-creatorはPhasegate本体メンテナー向けでありconsumerセットには配備しない。旧allと29件のcorpusは維持。descriptionは本体作業に限定し、一般PJのreleaseやskill作成に固有ルールを適用しない。カタログ更新指示はinstallationの一箇所を参照する。本文の全面書換えやモデル性能向上は本変更の範囲外。
