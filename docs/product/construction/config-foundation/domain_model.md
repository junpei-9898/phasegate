...
| **ModelRoutingConfig** | ✅ | ✅ | `modelRouting.delegation` の公開互換設定。`delegate-sonnet` / `none` の構造と schema validation を config-foundation が所有し、実行時の意味論は setup / harness-api / skill-quality が解釈する。<!-- @work-item-id WI-219 --> |
