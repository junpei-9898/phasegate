...
@work-item-id WI-251
バッチコミット帰属の精緻化（file-tag-scoped attribution）: `FileSystemStoryReflectionAdapter#extractChangedPathsForStory` に、複数 `Work-Item:` trailer を同梱するバッチコミット向けの帰属フィルタを追加。single-trailer コミットは従来どおり全 changed paths を帰属するが、複数 trailer コミットが触った `scripts/harness/{unit}/{layer}/` 配下の changed path は、そのファイルの現在内容（HEAD）の `@work-item-id`/`@story` タグ集合に `storyId` が含まれる場合のみ帰属する。タグ無し・ファイルが読めない（HEAD 削除等）・判定不能は fail-closed で帰属維持（反映要求を緩めない）。非ソースパス（`scripts/harness/{unit}/{layer}/` 外）は無条件帰属のまま。これにより WI-117/118/139 が所有する validator-system domain ファイルを根拠に WI-122/124/125/128/131 が domain-touch と誤判定される over-attribution を除去する。帰属は狭まる方向にのみ作用し（単調性）、`storyTouchesUnitLayer` の結果は true→false にしか変わらないため新規 violation は発生しない。port（`storyTouchesUnitLayer` シグネチャ）・checker・preset デフォルト（full-story-reflection-defaults.ts）は不変。
@work-item-id WI-126
