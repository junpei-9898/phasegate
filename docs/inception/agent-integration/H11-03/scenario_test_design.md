...
- PostToolUse Hook 発火時に `harness:lint --fast` コマンドを呼び出す（harness-api経由で統一）
...
- CLI経路でも `harness:lint` により同等機能が実行可能
...
| テストID | シナリオ | 入力 | 期待結果 |
|---------|---------|------|---------|
| SC-H11-03-001 | PostToolUse Hook が正常に `harness:lint --fast` を呼び出すこと | `toolName='Write'`, Hook有効, CLIExecutor: exitCode=0 | `executed=true`, `skipReason=undefined`, `cliResult.exitCode=0` |
...
- `HookToCliTranslator.translate(PostToolUseEvent)`: `{ shouldBlock: false, cliCommand: 'harness:lint', cliArgs: ['--fast'], expectedExitCode: 0, timeoutMs: 500 }`
