...
### Q1: 値オブジェクトテストの粒度
- **Q**: HookEvent / WriteTargetScope 等の VO を 1 ケース 1 テストにするか、まとめるか？
- **A**: **不変条件（INV-X）ごとに 1 テスト**。検証ポイントを明確化し、失敗時の特定を容易にする。
- **根拠**: プロジェクト共通のテスト規約（AAA + 日本語名 + 単一検証）。

### Q2: ドメインサービスのモック方針
- **Q**: HookToCliTranslator のテストで ReentryGuard を実装にするかモックにするか？
- **A**: **実装**。ReentryGuard は同じ Unit 内のドメインコンポーネントで、ドメイン層のモック禁止原則に従う。
- **根拠**: testing-rules.md のドメイン層モック禁止ルール。
