...
- **A**: **擬似コード**。実装時に細部は調整可能な粒度で記述。
- **根拠**: ロジック設計は実装方針の共有であり、コードの完全な事前記述ではない。

### Q2: Adapter テストのファイルシステム扱い
- **Q**: ファイルシステム I/O を含む Adapter テストで、一時ディレクトリをどう管理するか？
- **A**: **`os.tmpdir()` + テストごとに UUID ディレクトリ**。afterEach で確実に削除。
- **根拠**: テスト間の独立性確保とゴミ残留防止。
