...
| npm publish | **v0.91.0 published as latest (2026-04-23)**。v0.84.0〜v0.90.0 は intermediate で未 publish。v0.92.0〜v0.94.0 は設計文書のみ、v0.95.0 は Wave 2 VO 改修、v0.96.0 は Wave 3 schema v3 + config-foundation、v0.97.0 は Wave 4 biome-ast-engine 配線 + flat preset、v0.98.0 は Wave 5 pipeline 配線完成、v0.99.0 は Wave 5.5 dogfood integration test（publish は user 判断待ち） |
| 次の最優先候補 | **(A) ISSUE-014 Wave 6 — preset 選定ガイド + `migrate` CLI + 呼称分離ガイド** — README / retrofit-adoption.md / CLAUDE.md へ preset 選定ガイド追記、`phasegate migrate --schema v3` CLI、v0.86.0 未満からの境界警告、防御プリセット/アーキプリセット呼称分離。推定 0.5d / **(B) ISSUE-001 inception 設計順序強制** — AIDLC 完成度向上、設計スキップ物理ブロック、中規模 / (C) ISSUE-015 comment-only diff 検出（P3 / 小規模）/ (D) ISSUE-012 pre-commit 拡張フィルタ config（P3 / 小規模）/ (E) npm publish 判断（v0.92.0〜v0.99.0 の 8 バージョン） |
