...
| ISSUE-014 Wave 2 | **完了 (v0.95.0, 2026-04-23)**: `LayerName` / `LayerBoundary` VO を `ArchitectureSpec` 注入形式に改修。`scripts/harness/biome-ast-engine/domain/value-objects/architecture-spec.ts` を新設し `CLEAN_PRESET_SPEC` を default に設定（既存挙動完全維持）。onion preset 相当の custom spec 注入テスト 12 件追加（3311 → 3323）。計画の小穴: dogfood（phasegate.config.json の `architecture` 明示追記）を Wave 2 → Wave 3 に移送（v2 schema `additionalProperties: false` と抵触するため） |
| 現在の lint 状態 | **0 件** 🎉 維持。全 L1-001〜L1-007 violations 無し |
| 現在の validate-metadata 状態 | **0 件 FAIL** 🎉（全 105 Unit 構成文書 PASS） |
| npm publish | **v0.91.0 published as latest (2026-04-23)**。v0.84.0〜v0.90.0 は intermediate で未 publish（最新版への集約で運用上問題なし）。v0.92.0〜v0.94.0 は設計文書のみ、v0.95.0 は Wave 2 VO 改修（publish は user 判断待ち） |
| 次の最優先候補 | **(A) ISSUE-014 Wave 3 — schema v3 実装 + config-foundation 構造検出ロード + preset override 解決 + semantic validation (C1〜C5) + layerDetection precedence + phasegate.config.json dogfood 追記**（Wave 2 完了済。推定 1.5d / 最も論理的な次ステップ）/ **(B) ISSUE-001 inception 設計順序強制** — AIDLC 完成度向上、設計スキップ物理ブロック、中規模 / (C) ISSUE-015 comment-only diff 検出（P3 / 小規模）/ (D) ISSUE-012 pre-commit 拡張フィルタ config（P3 / 小規模） |
