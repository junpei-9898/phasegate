...
| npm publish | **v0.91.0 published as latest (2026-04-23)**。v0.84.0〜v0.90.0 は intermediate で未 publish。v0.92.0〜v0.94.0 は設計文書のみ、v0.95.0 は Wave 2 VO 改修、v0.96.0 は Wave 3 schema v3 + config-foundation、v0.97.0 は Wave 4 biome-ast-engine 配線 + flat preset、v0.98.0 は Wave 5 pipeline 配線完成（publish は user 判断待ち） |
| 次の最優先候補 | **(A) ISSUE-014 Wave 5.5 — 外部 dogfood（onion / hexagonal / layered の 3 preset）を `/tmp/phasegate-dogfood-*/` で実施** — Wave 5 配線完成済。pre-tool-use-hook が `/tmp/**/src/domain/**` write を blocking する問題の整理（hook の scope 絞り or 回避策合意）が前提。推定 0.25d / **(B) ISSUE-014 Wave 6 — preset 選定ガイド + `migrate` CLI + 呼称分離ガイド**、0.5d / **(C) ISSUE-001 inception 設計順序強制** — AIDLC 完成度向上、設計スキップ物理ブロック、中規模 / (D) ISSUE-015 comment-only diff 検出（P3 / 小規模）/ (E) ISSUE-012 pre-commit 拡張フィルタ config（P3 / 小規模） |
