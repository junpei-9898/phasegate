...
| npm publish | **v0.91.0 published as latest (2026-04-23)**。v0.84.0〜v0.90.0 は intermediate で未 publish（最新版への集約で運用上問題なし）。v0.92.0〜v0.94.0 は設計文書のみ、v0.95.0 は Wave 2 VO 改修、v0.96.0 は Wave 3 schema v3 + config-foundation（publish は user 判断待ち） |
| 次の最優先候補 | **(A) ISSUE-014 Wave 4 — `flat` preset 実装（L1-001〜004 自動無効化 + 残存 tag 扱い + preset/user 優先度）** — Wave 3 完了済の次ステップ。推定 0.5d（小規模） / **(B) ISSUE-001 inception 設計順序強制** — AIDLC 完成度向上、設計スキップ物理ブロック、中規模 / (C) ISSUE-015 comment-only diff 検出（P3 / 小規模）/ (D) ISSUE-012 pre-commit 拡張フィルタ config（P3 / 小規模） |
