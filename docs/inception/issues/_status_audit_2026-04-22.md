...
| ISSUE-014 Wave 1 | **着手 (v0.92.0 → v0.94.0, 2026-04-23)**: ADR-015 起票 + `wave1_schema_proposal.md` で preset 7 種（clean / strict-ddd / onion / hexagonal / layered / flat / custom）の schema / 層行列 / マイグレーション戦略を確定。v0.93.0 で批判的レビュー 12 項目補修（schema 識別・semantic validation C1〜C5・preset override・layerDetection precedence・呼称分離 等）、v0.94.0 で計画再同期（Wave 3 に override 解決ロジック追加、Wave 6 に呼称分離ガイド追加、推定 4.5d → 5d に調整）。実装コード変更なし、設計文書のみ。Wave 2〜6 で段階実装（推定 5d） |
| 現在の lint 状態 | **0 件** 🎉 維持。全 L1-001〜L1-007 violations 無し |
| 現在の validate-metadata 状態 | **0 件 FAIL** 🎉（全 105 Unit 構成文書 PASS） |
| npm publish | **v0.91.0 published as latest (2026-04-23)**。v0.84.0〜v0.90.0 は intermediate で未 publish（最新版への集約で運用上問題なし）。v0.92.0 は設計文書のみの変更のため publish 不要（user 判断待ち） |
| 次の最優先候補 | **(A) ISSUE-014 Wave 2 — `LayerName` VO の config 注入改修**（現在 ISSUE-014 Wave 1 完了済。Wave 2 は `LayerName` / `LayerBoundary` を config-foundation から注入受け付けに改修し、既存挙動を `clean` preset として維持する。推定 1d） / **(B) ISSUE-001 inception 設計順序強制** — AIDLC 完成度向上、設計スキップ物理ブロック、中規模 / (C) ISSUE-015 comment-only diff 検出（P3 / 小規模）/ (D) ISSUE-012 pre-commit 拡張フィルタ config（P3 / 小規模） |
