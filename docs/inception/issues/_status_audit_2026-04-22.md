...
| ISSUE-014 Wave 1 | **着手 (v0.92.0, 2026-04-23)**: ADR-015 起票 + `wave1_schema_proposal.md` で preset 7 種（clean / strict-ddd / onion / hexagonal / layered / flat / custom）の schema / 層行列 / マイグレーション戦略を確定。批判的レビューで穴補修済（schema 識別・semantic validation・preset override 優先度・layerDetection precedence 追加）。実装コード変更なし、設計文書のみ。Wave 2〜6 で段階実装（推定 4.5d） |
