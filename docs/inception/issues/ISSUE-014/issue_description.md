...
- **状態**: 🟡 **IN PROGRESS**（Wave 5 完了 / 2026-04-23）
  ...
  - Wave 5.5 (pending): 外部 dogfood `/tmp/phasegate-dogfood-onion / .../hexagonal / .../layered` の 3 preset 実機検証（pre-tool-use-hook の blocking 解消が前提）
...
- [ ] オニオン / ヘキサゴナル / flat の各プリセットで **外部** dogfood 実証（`/tmp/phasegate-dogfood-onion/` 等、Wave 5.5 で最低 3 preset — hook blocker の整理後に実施）
...
| Wave 3 ✅ | —（新設） | schema v3 JSON Schema 実体化 + 構造検出ロード + preset override 解決 + semantic validation (C1〜C5) + layerDetection precedence + **phasegate.config.json dogfood 追記**（完了 / v0.96.0） | 1.5d（実施: 0.75d） |
...
| Wave 5.5 | Phase E | 外部 dogfood（`/tmp/phasegate-dogfood-*/`）の 3 preset 検証 — pre-tool-use-hook blocker の整理が前提 | 0.25d |
