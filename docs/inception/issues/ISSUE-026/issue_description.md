...
- [ ] 全 work item が `docs/inception/{unit}/{WI-XXX}/` または `docs/inception/_cross/{WI-XXX}/` に統一される
- [ ] `docs/inception/issues/` と `docs/inception/{unit}/issues/` が廃止される
- [ ] WI frontmatter（`id` / `type` / `severity` / `affects`）が L2 metadata validator で検証される
- [ ] `type: story | issue | refactor` の WI について、`affects` 全 Unit の product 反映未完時に Level 3 書き込みがブロックされる
- [ ] `type: fix | chore` は description.md + PR trailer だけで証跡が残る軽量パスを提供する
- [ ] `@work-item-id` アノテーション（新）と `@story-id` / `@issue-id`（legacy）の両方で反映検出ができる
- [ ] `FileSystemStoryReflectionAdapter` が全 WI を uniform に列挙する（`_cross/` 含む、`{unit}/issues/` 特殊分岐なし）
- [ ] `WriteTargetScope.fromPath` が `_cross/{WI-XXX}/` と `{unit}/{WI-XXX}/` を Level 3 として扱う
- [ ] `git log --grep='Work-Item: WI-XXX'` で WI-ID に紐づく全コミットを遡れる
- [ ] 移行スクリプト `npx phasegate migrate work-items` が実行でき、`legacy_id` で旧 ID の grep 互換が保たれる
- [ ] Codex / Claude Code の両方で、少なくとも commit 前には未反映状態を fail にできる
- [ ] hook / metadata validator / consistency validator の責務分担が docs と実装で一致する
- [ ] 回帰テストで `story | issue | refactor | fix | chore` の各 type を検証する
