...
| G2-1 / G2-2 | ✅ 完了 | WI-027 (v0.105.0): planner / gateway を H-ID 検出に拡張し、57 件を WI-028..WI-084 に移行。その後 `_cross/WI-028..WI-036` との衝突を解消するため、該当する unit-scoped WI を WI-097..WI-105 に再採番。`FileSystemStoryReflectionAdapter#readLegacyId` を unit-scoped WI に対応させて post-migration dead-lock を解消 |
