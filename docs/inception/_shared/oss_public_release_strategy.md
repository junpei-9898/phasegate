...
- **作成日**: 2026-04-02
- **ステータス**: In Progress — Week 4 完了（npm publish を除く）
- **バージョン**: v1.5（2026-04-26 license を Apache-2.0 → MIT に変更）
...
**「MITで全公開 + 日本語コンテンツで認知獲得 + 導入コンサルで収益化」**
...
| 柱 | 内容 |
|---|---|
| **全公開で採用最大化** | 28スキル・5層防御モデルをMITで完全公開 |
...
> 2026-04-26 更新: Apache 2.0 から MIT に方針変更。理由は本節末尾「方針変更の経緯」を参照。

| ライセンス | 採用障壁 | コンサル誘引力 | ソロ開発者との相性 | 日本企業受容性 |
|-----------|---------|-------------|-----------------|-------------|
| **MIT** | **最低** | **中（差別化は知識で）** | **最高** | **高** |
| Apache 2.0 | 最低 | 中〜高 | 高 | 最高 |
...
1. **シンプルさが採用障壁を最小化する**: 1 ページの定型文で内容把握が即座に終わる。法務確認コストが Apache 2.0 より更に低く、個人〜中小規模での採用判断が即決される
2. **OSS エコシステムのデファクト**: npm エコシステムで最頻出のライセンス。依存関係に組み込む側の心理的抵抗が最も低い
3. **AGPLは日本企業が最も嫌うライセンス**: 法務が理解できず「使用禁止」にされるケースが多い。これは MIT/Apache どちらでも回避できる
4. **フォークリスクはこのプロジェクトでは低い**: 28スキル + 5層防御モデルの設計思想・運用知見はコードだけでは複製できない。コンサル価値は知識の非対称性で守る（License 条項では守らない）
5. **特許条項の不在は受容**: 個人開発者が原作者の本プロジェクトでは、Apache 2.0 の特許付与条項が機能する場面が限定的。エンタープライズで特許付与が必要になった段階で再検討する

### 絶対に避けるべきこと

- **ライセンス変更（Bait and Switch）**: 公開済みのライセンスを AGPL/BSL のような制限的ライセンスに変更するとコミュニティの信頼が壊滅する（HashiCorp, Redis, MongoDBの前例）。MIT へ緩和する方向の変更（今回のような Apache 2.0 → MIT）はこの懸念に該当しないが、今後 MIT より制限的なライセンスへの変更は行わない方針

### 方針変更の経緯（2026-04-26）

- v0.110.0 までは Apache 2.0 で公開。v0.111.0 以降は MIT に変更
- 個人開発者一人で運営する初期フェーズでは、Apache 2.0 の特許条項より MIT の単純さによる採用最大化の方が価値が高いと判断
- npm エコシステムでの心理的障壁を最小化し、Show HN / dev.to での認知獲得時に license 質問が発生しないようにする
- 既に Apache 2.0 で公開済みの v0.110.0 までのバージョンは Apache 2.0 のまま（過去 release は再ライセンスしない）
...
| 作業 | ファイル | 内容 | 状態 |
|------|--------|------|------|
| ライセンス追加 | `LICENSE` | MIT（v0.111.0 以降。v0.110.0 までは Apache 2.0） | ✅ |
...
### Week 1: 法的・メタデータ
- [x] `LICENSE` 作成 *(2026-04-02 Apache 2.0 で作成 → 2026-04-26 MIT に変更)*
...
### Week 4: セキュリティ・最終確認
- [x] 全 markdown のシークレットスキャン — 523ファイル検査、秘密情報なし *(2026-04-03 完了)*
- [x] ライセンス互換性チェック — 全依存が MIT 互換 (MIT/BSD-3/Apache-2.0) *(2026-04-03 完了)*
