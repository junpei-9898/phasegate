...
### Q1: Unit の粒度基準
- **Q**: 1 Unit あたりの US 数の目安は？
- **A**: **3〜6 US**。US 数が多すぎると単一集約では表現できず、少なすぎると Unit 間結合が増える。
- **根拠**: v0 の 13 Unit を分析した結果の実証値。

### Q2: 既存 Unit の v1 への引き継ぎ方針
- **Q**: v0 の 13 Unit をそのまま v1 で使うか、再設計するか？
- **A**: **再設計**。v1 の Epic 構成（H-01〜H-15）に沿って Unit を再定義する。v0 Unit は archive に保存。
- **根拠**: v1 では Phase Gate / Traceability / Preset 等の新機能を中核に据えるため、Unit 境界も変わる。

### Q3: 統合契約（integration_contract.md）の更新タイミング
- **Q**: Unit 間契約はいつ確定するか？
- **A**: **Unit 定義完了と同時**。Unit ごとに他 Unit へ公開する契約を明記し、統合契約ファイルで相互参照を管理。
- **根拠**: 契約が後付けだと Unit 間結合の制御が形骸化する。
