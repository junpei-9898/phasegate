# シナリオテスト設計: H10-01 — Quick Mode設定（phasegate.config.json quickModeセクション）
...
| SC-H10-01-005 | quickModeセクションなしのphasegate.config.jsonでデフォルト設定が適用される | phasegate.config.json（quickModeセクション省略） | allowedCategories=['bugfix','docs','test','config'] で動作 |
...
## 4. 前提条件
- `phasegate.config.json` のJSONスキーマバリデーション通過
