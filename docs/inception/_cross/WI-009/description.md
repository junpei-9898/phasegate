...
status: drafted
legacy_id: ISSUE-009
affects: [agent_integration（`HandlePreToolUseUseCase` が Claude 固有 hook に結合）, validator_system（`L0-002: fuse-mount-status` が未実装スロットとして予約されたまま放置）, 新規 Unit: `guard-system`（本 issue で提案）]
---
