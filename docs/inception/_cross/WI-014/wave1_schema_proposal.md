...
| **Wave 5.5** ✅ | 3 preset（onion / hexagonal / layered）の dogfood を vitest integration test（`preset-dogfood.integration.test.ts`）で自動化 — 許容/違反双方 6 テストで `createBiomeAstEngineModule` 経由 `ExecuteLintUseCase` を end-to-end 呼び出し、`os.tmpdir()` 配下にランタイム `fs.writeFileSync` で fixture 展開。`/tmp/**` への Claude Write は pre-tool-use-hook でブロックされるため、テストベース検証へ切替 — 完了 / v0.99.0 | 0.25d（実施: 0.2d） | Wave 5 |
| **Wave 6** ✅ | `docs/guide/preset-selection.md` 新設（7 preset 早見表 + 選択フロー + 設定例）、README.md / CLAUDE.md / retrofit-adoption.md に preset 選定 & 呼称分離ガイド追記、`phasegate migrate --schema v3` CLI 実装（`MigrateSchemaUseCase` + `MigrateSchemaCommandHandler` + `main.ts` dispatch + ヘルプ）、v0.86.0 未満相当の v2 schema 検出時に stderr 警告（`LoadResolvedConfigUseCase.schemaVersion` + 1-shot emit）— 完了 / v0.100.0 | 0.5d（実施: 0.4d） | Wave 5.5 |
