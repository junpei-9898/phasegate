...
報告テンプレートは [references/unit-test-design-template.md](references/unit-test-design-template.md) の「3. 上位設計不足時の報告テンプレート」を参照。
...
スコープ、テスト対象分析（集約・エンティティ・値オブジェクト）、テスト方針、QA、前提条件・リスクの5セクション構成。

詳細なテンプレートは [references/unit-test-design-template.md](references/unit-test-design-template.md) の「1. Phase 1 計画ファイルテンプレート」を参照。
...
対象ドメインモデル、集約テストケース（不変条件・状態遷移・生成）、エンティティテストケース（ビジネスルール・振る舞い）、値オブジェクトテストケース（等値性・制約）、境界値・異常系の5セクション構成。

詳細なテンプレートは [references/unit-test-design-template.md](references/unit-test-design-template.md) の「2. unit_test_design.md 成果物テンプレート」を参照。
